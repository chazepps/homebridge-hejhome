...
import {
  ...
  type SupportedDeviceModel,
} from '../devices/capabilities.js';

export type { SupportedDeviceModel } from '../devices/capabilities.js';
...
export function createDeviceSupportSummary(snapshot: HejDeviceSnapshot | null | undefined): DeviceSupportSummary {
  ...
  for (const device of devices) {
    ...
    if (capability?.supportStatus === 'partial') {
      partialCount += 1;
      continue;
    }
    if (capability?.supportStatus === 'deferred') {
      deferredCount += 1;
      continue;
    }
  ...
  return {
    ...
    partialCount,
    deferredCount,
    unsupportedCount: devices.length - supportedCount - partialCount - deferredCount,
